...
export async function GET(request: NextRequest) {
  ...
  const { data } = await s.supabase
    ...
    .select("seq,deadhead_km,unassigned_reason,fare,vehicles(plate_number),drivers(name,phone),bookings(booking_no,pickup_at,product_name,customer_name,customer_phone,pax,pickup_address,dropoff_address,flight_no,memo)")
    .eq("run_id", runId);
  ...
  type Row = {
    ...
    bookings: { booking_no: string | null; pickup_at: string | null; product_name: string | null; customer_name: string | null; customer_phone: string | null; pax: number; pickup_address: string | null; dropoff_address: string | null; flight_no: string | null; memo: string | null };
  };
  ...
  const csv = toCsv([
    ["이용일", "차량", "기사", "기사연락처", "순번", "픽업시간", "예약번호", "상품", "고객", "고객연락처", "인원", "픽업장소", "하차장소", "항공편", "메모", "공차이동(km)", "기사지급액", "배차불가사유"],
    ...rows.map((r) => [
      ...
      r.bookings.pax, r.bookings.pickup_address, r.bookings.dropoff_address, r.bookings.flight_no, r.bookings.memo,
