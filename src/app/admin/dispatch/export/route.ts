...
export async function GET(request: NextRequest) {
  ...
  const { data } = await s.supabase
    ...
    .select("seq,deadhead_km,unassigned_reason,fare,vehicles(plate_number),drivers(name,phone),bookings(booking_no,pickup_at,product_name,customer_name,customer_phone,pax,pickup_address,dropoff_address,flight_no,memo,pickup_place,dropoff_place,trip_type,vehicle_class)")
    .eq("run_id", runId);
  ...
  const csv = toCsv([
    ["이용일", "차량", "기사", "기사연락처", "순번", "시간", "구분", "예약번호", "차급", "고객", "고객연락처", "인원", "픽업장소", "픽업주소", "하차장소", "하차주소", "항공편", "메모", "공차이동(km)", "기사지급액", "배차불가사유"],
    ...rows.map((r) => [
      run.service_date, r.vehicles?.plate_number ?? "", r.drivers?.name ?? "", r.drivers?.phone ?? "", r.seq ?? "",
      fmtTime(r.bookings.pickup_at), tripLabel(r.bookings.trip_type)?.label ?? "", r.bookings.booking_no, r.bookings.vehicle_class ?? r.bookings.product_name, r.bookings.customer_name, r.bookings.customer_phone,
